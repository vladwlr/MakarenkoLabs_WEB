...
class EnergySystem {
    ...
    getBalance(period) {
        return this.items.reduce((sum, item) => sum + (period === 'day' ? item.dayPower() : item.nightPower()), 0);
    }
    makeReport(period) {
        const rows = [];
        ...
        rows.push(period === 'day' ? '--- День ---' : '--- Ніч ---');
        rows.push(`Початковий баланс: ${balance.toFixed(2)} МВт`);

        for (const line of this.lines) {
            if (Math.abs(balance) < 0.001) break;
            ...
            rows.push(`${line.title}: ${action} ${operation.amount.toFixed(2)} МВт`);
        }

        rows.push(`Залишок: ${balance.toFixed(2)} МВт`);
        rows.push(totalMoney >= 0 ? `Витрати: ${totalMoney.toFixed(2)} грн` : `Прибуток: ${Math.abs(totalMoney).toFixed(2)} грн`);
        return rows.join('\n');
    }
}
...
function renderReport() {
    ...
    document.getElementById('report').textContent = [
        grid.makeReport('day'),
        '',
        grid.makeReport('night')
    ].join('\n');
}

document.getElementById('runReport').addEventListener('click', renderReport);
renderReport();
